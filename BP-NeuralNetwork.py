...
font = fm.FontProperties(fname=r'./stheitisclight/STHeiti-Light.ttc')
...
class NeuralNetwork:
    ...
    def set_xy(self, input, expected_output):
        ...
        print("-----x值-------")
        print(self.x)
        print("x的价数%s" % str(self.x.shape))
        print("-----预期输出-----")
        print(self.y)
        print("y的价数%s" % str(self.y.shape))
    ...
    def init_w_and_b(self):
        ...
        for l in range(1, self.layers_num):
            ...
            print("-------self.w------")
            print("w%s的值" % str(l))
            print("w的类型%s" % type(self.w))
            print(self.w["w" + str(l)])
            print("-------self.b-------")
            print("b%s的值" % str(l))
            print(self.b["b" + str(l)])
            '''
    ...
    def layer_activation_forward(self, x, w, b, activation_choose):
     ...
     """
        ...
        print("-----------------------------------")
        print(input_sum)
        print("input_sum shape %s" % str(input_sum.shape))
        print("-----------------------------------")
        '''
        # 对输入加权和进行激活输出
    ...
    def training_modle(self):
        ...
        if False:
            ...
            plt.xlabel(u'迭代次数 (*500)', fontproperties = font)
...
#plt.subplot(5, 2, 5)
#plt.title(u'隐藏层神经元数量: 50', fontproperties=font)
